...
def train(cfg, train=True):
    ...
    cfg.SAVE_DIR = f'output/auc_d{cfg.DATA.DATASET_NAME}-{cfg.LOSSES.NAME}-MSsp{cfg.LOSSES.MULTI_SIMILARITY_LOSS.SCALE_POS}-MSsn{cfg.LOSSES.MULTI_SIMILARITY_LOSS.SCALE_NEG}-hm{cfg.LOSSES.MULTI_SIMILARITY_LOSS.HARD_MINING}-{cfg.MODEL.HEAD.DIM}-{cfg.MODEL.BACKBONE.NAME}-lr{cfg.SOLVER.BASE_LR}-bs{cfg.DATA.TRAIN_BATCHSIZE}/'
